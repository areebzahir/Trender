...
void main() {
    ...
    if (t > 0.0) {
        ...
        // Mix primary and accent colors based on position and time
        float mixFactor = sin(time * 0.5 + p.x * 2.0) * 0.5 + 0.5;
        color = mix(primaryColor, accentColor, mixFactor) * fresnel;
        
        gl_FragColor = vec4(color, 0.4); // Much more vibrant and visible
    } else {
...
function ThemedFluidBlobShader() {
  ...
  const uniforms = useMemo(() => ({
    ...
    primaryColor: { value: new THREE.Vector3(1.0, 0.3, 0.1) }, // Vibrant #FF4D1A - Blazing Orange
    accentColor: { value: new THREE.Vector3(1.0, 0.6, 0.0) }   // Electric #FF9900 - Golden Orange
  }), []);
